...
type EventListenerLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.EventListener, err error)
...
// List lists all EventListeners in the indexer.
func (s *eventListenerLister) List(selector labels.Selector) (ret []*v1beta2.EventListener, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.EventListener))
	})
	return ret, err
}
...
type EventListenerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.EventListener, error)
...
// List lists all EventListeners in the indexer for a given namespace.
func (s eventListenerNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.EventListener, err error) {
...
// Get retrieves the EventListener from the indexer for a given namespace and name.
func (s eventListenerNamespaceLister) Get(name string) (*v1beta2.EventListener, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("eventlistener"), name)
	}
	return obj.(*v1beta2.EventListener), nil
}
