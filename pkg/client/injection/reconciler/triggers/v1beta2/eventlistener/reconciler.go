...
import (
	...
	triggersv1beta2 "github.com/tektoncd/triggers/pkg/client/listers/triggers/v1beta2"
	zap "go.uber.org/zap"
...
// controller reconciling v1beta2.EventListener.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1beta2.EventListener. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1beta2.EventListener) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta2.EventListener.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1beta2.EventListener. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1beta2.EventListener) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1beta2.EventListener if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1beta2.EventListener.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1beta2.EventListener) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta2.EventListener if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1beta2.EventListener.
	// This method should not write to the API.
	//
	// Deprecated: Use reconciler.ObserveDeletion instead.
	ObserveFinalizeKind(ctx context.Context, o *v1beta2.EventListener) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1beta2.EventListener) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1beta2.EventListener resources.
type reconcilerImpl struct {
	...
	Lister triggersv1beta2.EventListenerLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister triggersv1beta2.EventListenerLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1beta2.EventListener, desired *v1beta2.EventListener) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.TriggersV1beta2().EventListeners(desired.Namespace)
		...
		updater := r.Client.TriggersV1beta2().EventListeners(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1beta2.EventListener) (*v1beta2.EventListener, error) {
	...
	patcher := r.Client.TriggersV1beta2().EventListeners(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1beta2.EventListener) (*v1beta2.EventListener, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1beta2.EventListener, reconcileEvent reconciler.Event) (*v1beta2.EventListener, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
